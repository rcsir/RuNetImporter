...
namespace Smrf.NodeXL.GraphDataProviders.Facebook
{
    ...
    public class FacebookUserNetworkAnalyzer : FacebookNetworkAnalyzerBase
    {
        ...
        )
        {
            ...
            if (bDownloadFirstPosts)
            {
                //sFirstPart += sLimitFirstPosts;
                oResults = DownloadFirstPosts(iNrOfFirstPosts, sFirstPart);
            }
            else if (bDownloadBetweenDates)
            {
                ...
                for (int i = 1; i <= iNrOfMultiqueries; i++)
                {
                    ...
                    for (int j = 0; j < iNrOfQueriesPerMultiquery; j++)
                    {
                        ...
                        iIndex = i * iNrOfQueriesPerMultiquery + j;
        ...
        private List<JSONObject> DownloadFirstPosts
        (
            int iNrOfFirstPosts,
        ...
        )
        {
            ...
            int iNrOfQueriesPerVertex = (int) Math.Ceiling((double)iNrOfFirstPosts / iNrOfFirstPostsPerQuery);
            ...
            for (int i = 1; i <= iNrOfMultiqueries; i++)
            {
                ...
                for (int j = 0; j < iNrOfIterationsPerQuery; j++)
                {
                    ...
                    {
                        iStartLimit = k * iNrOfFirstPostsPerQuery;
                        iLimitCount = iNrOfFirstPosts < (k + 1) * iNrOfFirstPostsPerQuery ? iNrOfFirstPosts - iStartLimit : iNrOfFirstPostsPerQuery;
        ...
        {            
            ...
            oNetworkDescriber.AddSentence(
                "The network is built upon "
                );

            if (bDownloadFirstPosts)
            {
                oNetworkDescriber.AddSentence(
                "the first "+iNrOfFirstPosts+" posts of each friend timeline."
                );
            }
            else if (bDownloadBetweenDates)
            {
                oNetworkDescriber.AddSentence(
                "posts between "+oStartDate.ToString()+" and "+oEndDate.ToString()+" of each friend timeline."
                );
            }
